...
Document doc;
      ...
      catch(UnsupportedCharsetException| OutOfMemoryError e) //because java is unicode and webpage is not
      {
          return null;
      }
    ...
    void crawl() throws MalformedURLException {
        ...
        while ((url = urlsQueue.poll()) != null) {
            ...
            System.out.println(Thread.currentThread().getName() + " popped " + url.getId() + " : " + url.getUrl() + " from the urlsQueue");
             Random r= new Random();       
         if( url.getDate().toString().contains("1905") ||url.getNews()==1 || url.getRank()>0 || r.nextFloat()<0.5 ){     
